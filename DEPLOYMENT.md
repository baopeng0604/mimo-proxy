...
代理内置了一个**只读**远程控制台（新增 `console.py`），包含：

- **状态指示**：顶栏绿点 = 运行正常；红点 = 服务不可达 或 近期日志有报错。
- **实时终端**：网页实时输出命令行日志（含历史缓冲，最多 1000 条），报错着色、自动滚动。

### 1. 启用

在项目根目录 `.env` 中设置 `CONSOLE_TOKEN`（访问口令）：

```env
CONSOLE_TOKEN=你的口令
```

重启代理后生效。未设置该变量时控制台可用但**无口令保护**（仅建议本机调试，公网部署前务必设置）。

### 2. 访问

```
http://<服务器IP>:8899/console
```

首次访问输入 `CONSOLE_TOKEN`，Cookie 记住后一段时间内免登录。状态点 + 实时日志均通过浏览器直接刷新，无需 SSH。

### 3. 安全说明

- 控制台为**纯只读**，无任何操作按钮，不影响代理请求主路径（请求报错照常透传、日志同步进 journalctl）。
- 口令经 `hmac.compare_digest` 校验，Cookie 为 HttpOnly、`SameSite=Lax`；WebSocket 握手同样校验口令，无法绕过页面直接连。
- 若 `8899` 已对公网开放，建议配合防火墙仅放行可信 IP，并将 `CONSOLE_TOKEN` 设置成足够强的随机串。
